...
def main():
    ...
    acc = {c: [] for c in args.categories}

    with open(out, "a", newline="") as f:
        w = csv.writer(f)
        if new:
            w.writerow(["categoria", "repeticion", "delta_W", "n_alto", "n_bajo"])
        for cat in args.categories:
            elf = os.path.join(HERE, f"{cat}.elf")
            if not os.path.exists(elf):
                print(f"{cat}: no existe {elf} (corre 'make {cat}.elf')")
                continue
            for r in range(1, args.repeats + 1):
                ...
                seen = len(rows)
                delta = row["p_avg"]                  # el delta va en p_avg
                nH, nL = row["samples"], row["duration_ms"]
                w.writerow([cat, r, delta, nH, nL]); f.flush()
                acc[cat].append(delta)
                print(f"    DELTA = {delta*1000:.3f} mW  (nH={nH} nL={nL})")
                time.sleep(5)

    print("\n=== Potencia dinamica por categoria (medida fisica, chopper) ===")
