...
| Script | Función |
|---|---|
| `caracterizar.py` | Corre una campaña completa de un método (`--metodo bucles` o `regresion`) y ajusta los coeficientes. Cada campaña respalda sus coeficientes en `<metodo>/campanas/coeficientes_<timestamp>.csv`. |
| `verificar.py` | Valida los coeficientes contra las 14 cargas de `benchmarks/`. `--pidle archivo` usa la potencia estática de la calibración; `--pidle medir` la mide en el momento (recomendado si la temperatura ambiente cambió). `--repeats N` promedia N tandas. Cada tanda queda en `validaciones/`. |
