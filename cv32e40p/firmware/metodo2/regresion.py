...
def build_design(runs):
    """Construye la matriz de diseno A (histogramas), el vector de energia b
    (Joules) y la duracion T (s) de cada ejecucion."""
    names, A, b, Ts = [], [], [], []
    for name, pbar, w in runs:
        ...
        Ts.append(T)
        names.append(name)
    return names, np.array(A, dtype=float), np.array(b, dtype=float), Ts


def fit_coefficients(A, b):
    """Ajusta e_i por minimos cuadrados. Categorias sin ninguna ejecucion
    que las ejercite -> columna vacia (singular); se resuelven solo las
    columnas con datos y el resto queda en 0 ("sin datos")."""
...
def main():
    ...
    names, A, b, Ts = build_design(runs)
    e, dropped, Ar = fit_coefficients(A, b)
