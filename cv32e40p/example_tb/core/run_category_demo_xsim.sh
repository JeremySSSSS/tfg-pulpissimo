...
LOG_DIR=${LOG_DIR:-xsim_category_demo_logs}
FIRMWARE_HEX=${FIRMWARE_HEX:-custom/category_counter_demo.hex}
...
sed '/^+incdir/d; s#${DESIGN_RTL_DIR}#'"${DESIGN_RTL_DIR}"'#g' \
  "${CORE_ROOT}/cv32e40p_manifest.flist" > /tmp/cv32e40p_xsim_demo.flist

sed 's/\.PULP_XPULP[[:space:]]*(PULP_XPULP)/.COREV_PULP      (PULP_XPULP)/; s/\.PULP_CLUSTER[[:space:]]*(PULP_CLUSTER)/.COREV_CLUSTER   (PULP_CLUSTER)/' \
  cv32e40p_tb_subsystem.sv > /tmp/cv32e40p_tb_subsystem_demo_xsim.sv
...
xvlog -sv -log "${LOG_DIR}/xvlog.log" \
  ...
  -i "${CORE_ROOT}/sva" \
  -f /tmp/cv32e40p_xsim_demo.flist \
  include/perturbation_pkg.sv \
  amo_shim.sv \
  cv32e40p_random_interrupt_generator.sv \
  /tmp/cv32e40p_tb_subsystem_demo_xsim.sv \
  dp_ram.sv \
  mm_ram.sv \
  riscv_gnt_stall.sv \
...
xelab tb_top -debug typical -s tb_top_demo_behav -log "${LOG_DIR}/xelab.log"

RUN_LOG="${LOG_DIR}/category_demo.log"

echo "==> Running ${FIRMWARE_HEX}"
xsim tb_top_demo_behav -R -log "${RUN_LOG}" \
  --testplusarg firmware="${FIRMWARE_HEX}" \
  --testplusarg maxcycles=300000

grep -E "category counter demo|arith=.*logic=.*memory=.*branch=.*jump=.*float=.*|EXIT SUCCESS" "${RUN_LOG}" || true

if ! grep -q "arith=10 logic=2 memory=4 branch=1 jump=1 float=0" "${RUN_LOG}"; then
  echo "FAIL: expected demo counts not found. See ${RUN_LOG}"
  exit 1
fi
...
echo "PASS: demo program completed"
echo "Log: ${RUN_LOG}"
